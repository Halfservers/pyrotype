        ...
        >
          <div className='flex flex-col text-left'>
            ...
            {selectedVersion?.files?.[0]?.size && (
              <span className='text-xs text-gray-400'>({formatFileSize(selectedVersion.files[0].size)})</span>
            )}
          </div>
            ...
            {versions.map((version) => (
              <div
                ...
                className={`px-4 py-3 cursor-pointer transition-colors ${
                  version.id === selectedVersion?.id ? 'bg-blue-600 text-white' : 'hover:bg-gray-700'
                }`}
              ...
              >
                <div className='flex flex-col'>
                  ...
                  {version.name && <span className='text-sm text-gray-300 truncate'>{version.name}</span>}
                  <div className='flex gap-2 mt-1 text-xs text-gray-400'>
                    {version.files?.[0]?.size && <span>Size: {formatFileSize(version.files[0].size)}</span>}
                    {version.game_versions?.length > 0 && <span>Game: {version.game_versions[0]}</span>}
                  </div>
                </div>
              </div>
            ))}
