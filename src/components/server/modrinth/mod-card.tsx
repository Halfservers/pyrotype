...
export const ModCard = ({ mod }: ModCardProps) => {
  return (
    <div className='group bg-gradient-to-br from-[#090909] via-[#0f0f0f] to-[#131313] rounded-xl overflow-hidden border border-gray-800/70 hover:border-blue-500/60 transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/15 backdrop-blur-sm'>
      <div className='p-6 flex items-start space-x-5'>
        ...
        <div className='flex-1 min-w-0 space-y-3'>
          <div>
            <Link
              to={`${mod.id}` as any}
              className='text-xl font-bold text-white hover:text-blue-400 transition-colors duration-200 line-clamp-1 group-hover:underline'
            >
              {mod.title}
            </Link>
          ...
          <div className='flex items-center space-x-6 text-sm'>
            <div className='flex items-center space-x-2 text-gray-400'>
              ...
              <span className='font-semibold text-gray-300'>downloads: {formatDownloads(mod.downloads)}</span>
            </div>
            <div className='flex items-center space-x-2 text-gray-400'>
        ...
        <div className='flex-shrink-0 self-center'>
          <Button variant='outline' className='border-gray-500/70 border-2 hover:border-blue-500/50 hover:text-gray-200'>
