...
const subuserSchema = z.object({
  email: z.string().email('A valid email address must be provided.').max(191, 'Email must not exceed 191 characters.'),
  permissions: z.array(z.string()),
});

type SubuserFormValues = z.infer<typeof subuserSchema>;

interface Props {
  subuser?: Subuser;
  onSuccess: (subuser?: any) => void;
  onCancel: () => void;
  flashKey: string;
...
const UserFormComponent = ({ subuser, onSuccess, onCancel, flashKey, isSubmitting, setIsSubmitting }: Props) => {
  ...
  const onSubmit = (values: SubuserFormValues) => {
    ...
    const params = subuser
      ? { email: values.email, permissions: values.permissions, subuser }
      : { email: values.email, permissions: values.permissions };
  ...
  const toggleAllPermissions = () => {
    ...
    form.setValue('permissions', allSelected ? [] : [...editablePermissions]);
  };
  ...
  const toggleCategoryPermissions = (key: string) => {
    ...
    if (allSelected) {
      form.setValue('permissions', watchedPermissions.filter((p) => !categoryPermissions.includes(p)));
    } else {
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} placeholder='Enter the email of the user to invite' />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
