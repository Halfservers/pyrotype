...
const BackupContextMenu = ({ backup }: Props) => {
  ...
  const doDeletion = async () => {
    if (!deletePassword) {
      addFlash({ key: 'backup:delete', type: 'error', message: 'Password is required.' });
      return;
    }
    if (hasTwoFactor && !deleteTotpCode) {
      addFlash({ key: 'backup:delete', type: 'error', message: 'Two-factor code is required.' });
      return;
    }
  ...
  const doRestorationAction = async () => {
    if (!restorePassword) {
      addFlash({ key: 'backup:restore', type: 'error', message: 'Password is required.' });
      return;
    }
    if (hasTwoFactor && !restoreTotpCode) {
      addFlash({ key: 'backup:restore', type: 'error', message: 'Two-factor code is required.' });
      return;
    }
    ...
    <>
      <Dialog open={modal === 'rename'} onOpenChange={(open) => !open && setModal('')}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={doRename} disabled={!newName.trim() || newName.trim() === backup.name}>Rename</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={modal === 'unlock'} onOpenChange={(open) => !open && setModal('')}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This backup will no longer be protected from deletions.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={modal === 'restore'} onOpenChange={(open) => { if (!open) { setModal(''); setRestorePassword(''); setRestoreTotpCode(''); } }}>
        <DialogContent>
          ...
          <div className='space-y-4'>
            <p className='text-sm text-zinc-400'>Your server will be stopped during restoration.</p>
            <div className='p-4 bg-red-500/10 border border-red-500/20 rounded-lg'>
              <p className='text-sm text-red-300 font-medium'>Destructive Action</p>
              <p className='text-xs text-red-400 mt-1'>All current files will be replaced with backup data.</p>
            </div>
            <div>
          ...
          <DialogFooter>
            ...
            <Button variant='destructive' onClick={doRestorationAction} disabled={countdown > 0 || loading}>
              {loading ? 'Restoring...' : countdown > 0 ? `Restore (${countdown}s)` : 'Delete All & Restore'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={modal === 'delete'} onOpenChange={(open) => { if (!open) { setModal(''); setDeletePassword(''); setDeleteTotpCode(''); } }}>
        <DialogContent>
          ...
          <div className='space-y-4'>
            <p className='text-sm text-zinc-300'>This is a permanent operation.</p>
            <div>
