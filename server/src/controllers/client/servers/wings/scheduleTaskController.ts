...
async function resolveServerAndSchedule(c: AppContext) {
...
export async function store(c: AppContext) {
  ...
  const { server, schedule } = await resolveServerAndSchedule(c)
  ...
  return c.json(fractalItem('schedule_task', transformTask(task)))
}
...
export async function update(c: AppContext) {
  ...
  const body = taskSchema.parse(await c.req.json())
  const { server, schedule } = await resolveServerAndSchedule(c)
  const taskId = parseInt(c.req.param('task'), 10)
  ...
  return c.json(fractalItem('schedule_task', transformTask(updated)))
}

export async function deleteFn(c: AppContext) {
  const prisma = c.var.prisma
  const { schedule } = await resolveServerAndSchedule(c)
  const taskId = parseInt(c.req.param('task'), 10)
